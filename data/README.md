...
| `routes_cache.json` | Every resident's route (car and on foot) to every qualifying safe point, and the crews' routes, planned at the scenario time by `pnpm data:routes`. Coordinates, directions and geometry only | Yes, for the sample registry |
