...
- Calls (#8): once a zone's order is approved, **Call residents** phones every pending resident through SLNG and turns unanswered calls into *no answer*. There is no SIP trunk yet, so this stays off (`HACKFIRE_PHONE_CALLS`), and the coordinator takes a resident's call in the browser instead: select the resident → **Take … call here**.
