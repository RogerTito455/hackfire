...
- Backend with the five agent tools, all real, an in-memory triage state and a rescue queue prioritised by predicted time to impact.
...
- Predicted spread: a cone from the fire front's direction and speed over the last hours of hotspots, hour by hour up to 6 h, and the places it reaches (134 from OpenStreetMap, including care homes and schools) with their time to impact. `get_fire_status` answers from it.
...
Still to build: lead time, the voice agent and outbound calls, crew alerts by SMS. See [PLAN.md](PLAN.md) and the issues.
