...
5. **Calls from the dashboard** (#8). Without a phone: approve the resident's zone order, select the resident on the dashboard → **Take … call here**. The backend opens a web session with that resident's four variables, so the agent reports on the right pin (the SLNG Test agent panel always talks as `n01`).
