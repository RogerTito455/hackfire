...
4. Check the voice agent answers (#7). Without a phone line (no SIP trunk, `HACKFIRE_PHONE_CALLS` off), the call runs in the browser: approve La Atalaya's order, click a resident's pin → **Talk to the agent → Take … call here**, allow the microphone, and someone answers as the resident. The pin changes within two seconds of the agent recording the answer. With a trunk, **Call residents** on the approved order rings every resident's phone instead.
