...
- **What it does.** As the slider moves, the backend plays the script in `data/demo_timeline.json` (`backend/app/autopilot.py`), timed by the forecast rather than by the clock: a zone's order is approved a set number of minutes after the zone first enters the predicted spread (`impact.py`; La Atalaya and El Tiemblo both enter at 15:30 CEST), and each resident's call lands a set number of minutes after their zone's order. Residents start evacuating, one does not answer, one needs rescue (rescue queue and crew alert on the dashboard) and the one who did not answer is reached on a retry. A zone the forecast never reaches is not called. Scrubbing back undoes it. Residents are named by their position in the registry, so the same script works with the sample and the deployed registry.
- **The call card.** When a scripted outcome that has a recorded transcript lands, a **Llamada simulada** card at the top of the sheet types out, turn by turn, a real call of our resident agent with a resident simulated by Galtea (`data/demo_calls.json`), labelled *Respuestas reales de nuestro agente de voz ante un vecino simulado (Galtea)*, and ends with the status the agent recorded. It names the demo resident whose pin the scripted outcome set, and the time. Today only **needs rescue** has transcripts (the wheelchair user, then the resident who refuses to leave): in the 19 September runs the agent did not record the evacuating and wrong-number calls (docs/services/galtea.md), so those outcomes show no card rather than a call that ended another way.
- **The map follows the rescue.** When the scripted rescue lands, the autopilot sets the agent focus (`/api/focus`), so the dashboard selects that resident and draws the crew's route, as when the coordinator agent asks for it. Scrubbing back before it clears the focus.
...
- **Turning it off** puts back the residents, orders, alerts and agent focus as they were before it was switched on. **Reset demo** also turns it off and clears everything.
