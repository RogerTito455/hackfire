...
1. **What to avoid:** for residents, every hotspot observed up to the scenario time (`HACKFIRE_SCENARIO_TIME`, default 23 July 18:30 UTC), buffered by 750 m and merged (`replay.burned_area_m`), **plus one hour of predicted spread** (`spread.py`). For crews, only what has burned; if that blocks every road, they get the direct route with a spoken warning. The route clips it to a 14 km square around its midpoint and cuts 500 m clearings around the start and end, so the endpoints stay routable. At the default time the clipped area for La Atalaya → Cebreros is 10.7 km², 12.5 × 9.4 km.
2. **Where to go:** each resident goes to the nearest safe point in `data/places.json` that the predicted spread does not reach within 6 h and that is at least 3 km from the fire. At the default time that is Navahondilla for La Atalaya. No safe route gives a plain "no route" answer (the El Tiemblo residents at the default time). See [the finding](../findings/2026-09-19-evacuation-destinations.md). The crew's rescue routes start at the El Tiemblo fire station (OSM node 5582810191).
