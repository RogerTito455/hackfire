...
4. **Road closures** (`backend/app/closures.py`, `POST /api/closures`): the coordinator taps a cut road on the map, and a 150 m circle there joins the area every route avoids, crews' included (a crew's fallback ignores the fire, never a closure). The route cache is keyed by the closures in force, so no route from before a closure is served after it. With a road closed, a resident without an order goes to whichever of the three nearest safe points is fastest to reach; if a closure cuts off an order's destination, the directions say so and send them to the fastest one left. Closing a road names the residents already evacuating by a route through it, to call again. **Routes after a closure are planned live**, so they need `ORS_API_KEY`: the cached demo routes do not cover them.
5. **Cache:** memory, then `data/routes_cache.json`, then ORS. `pnpm data:routes` plans the residents' missing routes at the scenario time. Rerun it after changing the registry, the scenario time or `places.json`.
