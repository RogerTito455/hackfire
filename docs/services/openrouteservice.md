...
1. **What to avoid:** every hotspot observed up to the scenario time (`HACKFIRE_SCENARIO_TIME`, default 23 July 18:30 UTC), buffered by 750 m and merged (`replay.burned_area_m`). The route clips it to a 14 km square around its midpoint and cuts 500 m clearings around the start and end, so the endpoints stay routable. At the default time the clipped area for La Atalaya → Cebreros is 10.7 km², 12.5 × 9.4 km.
2. **Where to go:** residents go to the safe point in `data/places.json` farthest from the burned area. At the default time that is Cebreros, 6 km from the fire; El Tiemblo is 1 km away and San Martín de Valdeiglesias 0.4 km. The crew's rescue routes start at the El Tiemblo fire station (OSM node 5582810191).
