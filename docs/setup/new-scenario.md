...
8. **Registry:** `pnpm data:registry` writes the tracked one from OpenStreetMap: ten households on real streets of the scenario's `places`, labelled by their street, with placeholder phones. Give each place an `address` first (how an address there ends, after the street). Residents already in the file keep their id and coordinates, so rerunning it only fills the gaps. If the team wants real calls, add a local one (`local_registry`, not tracked) or `HACKFIRE_NEIGHBORS_JSON`. Each resident's `zone` must be a zone id.
...
| `data:zones`, `data:registry` | Overpass | none |
