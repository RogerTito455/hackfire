...
avoids the area burned up to the scenario time plus the next AVOID_AHEAD_H of predicted spread. A
rescue route goes from the crew base to the resident's home and avoids only what has burned: crews
work inside the fire's path. If even that leaves no way through, the crew still gets the direct
route, with a warning. Routes are cached in memory and in data/routes_cache.json (written by `pnpm data:routes`),
...
# Avoid where the fire is predicted to be within this many hours, not only where it has been.
AVOID_AHEAD_H = 1
# A safe point must be at least this far from the burned area.
...
def _is_safe(place: Place, at: datetime) -> bool:
    area = geo.point_m(place.lon, place.lat).buffer(1_000)
    return (
        replay.burned_area_m(at).distance(area) >= SAFE_DISTANCE_M
        and spread.minutes_to_impact(area, at) is None
    )


def safest_point(at: datetime, start: tuple[float, float] | None = None) -> Place:
    """The nearest safe point the predicted spread does not reach and that is clear of the fire.
...
def avoid_polygon(
    ...
    The area burned by `at` plus `ahead_h` hours of predicted spread.
    """
    a, b = geo.point_m(*start), geo.point_m(*end)
    middle = Point((a.x + b.x) / 2, (a.y + b.y) / 2)
    area = (
        spread.predicted_area(at, ahead_h)
...
def fire_area(at: datetime | None = None, crew: bool = False) -> dict:
    ...
    return {
        "type": "Feature",
        "geometry": mapping(geo.to_degrees(spread.predicted_area(at, ahead_h))),
